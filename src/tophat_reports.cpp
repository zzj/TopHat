                        ...
                        )
{
    ...
    { //only right read was mapped properly
        //v1.4.1 - gave up trying to keep paired reads in sync, it can't be done now
        //because prep_reads may have discarded some of them anyway
        /*
        if (right_um_out) {
               ...
               curr_right_obs_order != VMAXINT32)
        {
            ...
            */
            got_read=get_read_from_stream(curr_right_obs_order,
                    left_reads,reads_format, false, l_read, left_um_out, true);
            //assert(got_read);
